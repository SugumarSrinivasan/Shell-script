...
function check_passwordless_ssh() {
  local msg="System: User:$user_account has Passwordless SSH Enabled"
  local server_name=`hostname -f`
  local ssh_status=`ssh $user_account@$server_name "whoami" &>/dev/null; echo $?`
#  echo $ssh_status
  if [ $ssh_status -eq 0 ]; then
       state "$msg" 0
  else
       state "System: User:$user_account don't have Passwordless SSH Enabled" 1
  fi
}
