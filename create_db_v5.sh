...
if [ $# -eq 0 ]; then
    ...
    echo "4 for updating the root_context in MYSQL."
    echo "5 for create database in Hive."
    echo "6 for create schema in HBase through Phoenix shell."
    echo "7 for execute migration script for hive and hbase."
    echo
    exit 1
else
    case $OPTION in
    ...
    3)
        ...
        ;;     
    4)
        echo "*****Updating the root_context in MYSQL.*****"
        echo
        read -p "Database server:" DB_SERVER
        echo
        read -p "Database user name:" DB_USER_NAME
